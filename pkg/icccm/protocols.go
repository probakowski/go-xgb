...
// func IsDeleteProtocol(xconn *xgb.XConn, ev xevent.ClientMessageEvent) bool {
...
// 	typeName, err := xprop.AtomName(xconn, ev.Type)
// 	if err != nil || typeName != "WM_PROTOCOLS" { // not what we want
// 		return false
// 	}

// 	// Check to make sure the first data item is WM_DELETE_WINDOW.
// 	protocolType, err := xprop.AtomName(xconn, xproto.Atom(ev.Data.Data32[0]))
...
// func IsFocusProtocol(xconn *xgb.XConn, ev xevent.ClientMessageEvent) bool {
// 	// Make sure the Format is 32. (Meaning that each data item is
// 	// 32 bits.)
// 	if ev.Format != 32 {
// 		return false
