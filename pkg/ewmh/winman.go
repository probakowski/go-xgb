...
func GetEwmhWM(xu *xgbutil.XUtil) (string, error) {
	...
	if childCheck != childCheck2 {
		return "", fmt.Errorf("GetEwmhWM: _NET_SUPPORTING_WM_CHECK value on the root window (%x) does not match _NET_SUPPORTING_WM_CHECK value on the child window (%x).", childCheck, childCheck2)
	}
