...
func (x *XMLExpression) Translate() Expression {
	...
	log.Panicf("Unrecognized tag '%s' in expression context. Expected one of op, fieldref, value, bit, enumref, unop, sumof or popcount.", x.XMLName.Local)
	panic("unreachable")
}
...
func (x *XMLField) Translate(parent interface{}) Field {
	...
	case "switch":
		if len(x.Bitcases) == 0 {
			log.Panic("switch field element with no bitcases")
		}
		swtch := &SwitchField{
			xmlName:  x.Name,
			MaskName: x.Expr.Translate(),
			MaskType: newTranslation("CARD32"),
