...
func (c *Context) Morph(xmlBytes []byte) {
	...
	if c.protocol.isExt() {
		...
		c.Putln("	eventFuncs = map[uint8]xgb.EventUnmarshaler{}")
		c.Putln("	errorFuncs = map[uint8]xgb.ErrorUnmarshaler{}")
		c.Putln(")")
		c.Putln("")
		...
		c.Putln("	extEventFuncs := map[uint8]xgb.EventUnmarshaler{}")
		...
		c.Putln("	extErrorFuncs := map[uint8]xgb.ErrorUnmarshaler{}")
