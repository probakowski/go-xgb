...
func (f *ValueField) Read(c *Context, prefix string) {
	...
	c.Putln("		%s%s[i] = binary.LittleEndian.Uint32(buf[b:])", prefix, f.ListName)
	c.Putln("		b += 4")
	c.Putln("	}")
...
func (f *ValueField) Write(c *Context, prefix string) {
	...
	c.Putln("	for i := 0; i < %s; i++ {", f.ListLength().Reduce(prefix))
	c.Putln("		binary.LittleEndian.PutUint32(buf[b:], %s%s[i])", prefix, f.ListName)
	c.Putln("		b += 4")
	c.Putln("	}")
	c.Putln("	b = internal.Pad4(b)")
}
...
func (f *SwitchField) Define(c *Context) {
	// no need to define, it as a uint32 that representing any enum
}

func (f *SwitchField) Read(c *Context, prefix string) {
	c.Putln("	%s%s = make([]uint32, %s)", prefix, f.SrcName(), f.ListLength().Reduce(prefix))
	c.Putln("	for i := 0; i < %s; i++ {", "")
	c.Putln("		%s%s[i] = binary.LittleEndian.Uint32(buf[b:])", prefix, f.SrcName())
	c.Putln("		b += 4")
	c.Putln("	}")
	c.Putln("	b = internal.Pad4(b)")
}
...
func (f *SwitchField) Write(c *Context, prefix string) {
	c.Putln("	for i := 0; i < len(%s%s); i++ {", prefix, f.SrcName())
	c.Putln("		binary.LittleEndian.PutUint32(buf[b:], %s%s[i])", prefix, f.SrcName())
	c.Putln("		b += 4")
	c.Putln("	}")
	c.Putln("	b = internal.Pad4(b)")
}
