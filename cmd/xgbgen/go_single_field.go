...
func ReadSimpleSingleField(c *Context, name string, typ Type) {
	switch t := typ.(type) {
	case *Resource:
		c.Putln("	%s = %s(binary.LittleEndian.Uint32(buf[b:]))", name, t.SrcName())
	case *TypeDef:
		switch t.Size().Eval() {
		case 1:
			c.Putln("	%s = %s(buf[b])", name, t.SrcName())
		case 2:
			c.Putln("	%s = %s(binary.LittleEndian.Uint16(buf[b:]))", name, t.SrcName())
		case 4:
			c.Putln("	%s = %s(binary.LittleEndian.Uint32(buf[b:]))", name, t.SrcName())
		case 8:
			c.Putln("	%s = %s(binary.LittleEndian.Uint64(buf[b:]))", name, t.SrcName())
		}
	case *Base:
		// If this is a bool, stop short and do something special.
		if t.SrcName() == "bool" {
			c.Putln("	%s = (buf[b] == 1)", name)
			break
		}
		...
		case 2:
			val = fmt.Sprintf("binary.LittleEndian.Uint16(buf[b:])")
		case 4:
			val = fmt.Sprintf("binary.LittleEndian.Uint32(buf[b:])")
		case 8:
			val = fmt.Sprintf("binary.LittleEndian.Uint64(buf[b:])")
		}
...
func WriteSimpleSingleField(c *Context, name string, typ Type) {
	switch t := typ.(type) {
	case *Resource:
		c.Putln("	binary.LittleEndian.PutUint32(buf[b:], uint32(%s))", name)
	case *TypeDef:
		switch t.Size().Eval() {
		case 1:
			c.Putln("	buf[b] = uint8(%s)", name)
		case 2:
			c.Putln("	binary.LittleEndian.PutUint16(buf[b:], uint16(%s))", name)
		case 4:
			c.Putln("	binary.LittleEndian.PutUint32(buf[b:], uint32(%s))", name)
		case 8:
			c.Putln("	binary.LittleEndian.PutUint64(buf[b:], uint64(%s))", name)
		}
	case *Base:
		// If this is a bool, stop short and do something special.
		if t.SrcName() == "bool" {
		...
		case 2:
			if t.SrcName() != "uint16" {
				...
				c.Putln("	binary.LittleEndian.PutUint16(buf[b:], %s)", name)
			}
		case 4:
			if t.SrcName() != "uint32" {
				c.Putln("	binary.LittleEndian.PutUint32(buf[b:], uint32(%s))", name)
			} else {
				c.Putln("	binary.LittleEndian.PutUint32(buf[b:], %s)", name)
			}
		case 8:
			if t.SrcName() != "uint64" {
				c.Putln("	binary.LittleEndian.PutUint64(buf[b:], uint64(%s))", name)
			} else {
				c.Putln("	binary.LittleEndian.PutUint64(buf[b:], %s)", name)
			}
		}
	default:
